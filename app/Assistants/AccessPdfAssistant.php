...
class AccessPdfAssistant extends PdfClient
{
    ...
    public static function validateFormat (array $lines) {
        ...
        $header_ok = trim($lines[0]) === "Access Logistic GmbH, Amerling 130, A-6233 Kramsach";
        if (!$header_ok) {
            return false;
        }

        $has_to_line = array_find_key($lines, fn($l) => Str::startsWith(trim($l), 'To:')) !== null;
        $has_contact = array_find_key($lines, fn($l) => Str::startsWith(trim($l), 'Contactperson:')) !== null;

        return $has_to_line && $has_contact;
    }
