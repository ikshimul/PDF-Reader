...
class DelamodePdfAssistant extends PdfClient
{
    ...
    public static function validateFormat (array $lines) {
        ...
        // Handle ASCII-fallback text extraction (without diacritics) and spacing differences
        $first = trim($lines[0]);
        $is_order_header = Str::startsWith($first, 'Pervezimo uzsakymas Nr.')
            || Str::startsWith($first, 'Pervežimo užsakymas Nr.');

        if (!$is_order_header) {
            return false;
        }

        $has_carrier = array_find_key($lines, function ($l) {
            $t = trim($l);
            // Accept when labels are combined on a single line (e.g., "Vezjas: Pastabos: Data:")
            return Str::contains($t, 'Vežėjas:') || Str::contains($t, 'Vezjas:');
        }) !== null;

        $has_notes_or_goods = array_find_key($lines, function ($l) {
            $t = trim($l);
            return Str::contains($t, 'Pastabos:') || Str::contains($t, 'Kroviniai');
        }) !== null;

        // Company line may be missing from the early text, do not require it for format validation
        return $has_carrier && $has_notes_or_goods;
    }
