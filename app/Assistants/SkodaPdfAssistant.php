...
class SkodaPdfAssistant extends PdfClient
{
    public static function validateFormat (array $lines) {
        ...
        $has_datum = array_find_key($lines, fn($l) => Str::startsWith(trim($l), 'DATUM: ')) !== null;

        // Normalize potential encoding issues where diacritics become replacement characters
        $has_loading_list = array_find_key($lines, function ($l) {
            $t = trim($l);
            if ($t === "NÁLOŽNÍ LIST / VERLADESCHEIN / LOADING LIST") {
                return true;
            }
            // Fallback when diacritics are lost or garbled
            return Str::contains($t, 'VERLADESCHEIN') && Str::contains($t, 'LOADING LIST');
        }) !== null;

        $has_customer_no = array_find_key($lines, function ($l) {
            $t = trim($l);
            if ($t === 'ODBĚRATEL / ABNEHMER / CUSTOMER NO.') {
                return true;
            }
            return Str::contains($t, 'ABNEHMER') && Str::contains($t, 'CUSTOMER NO');
        }) !== null;

        $has_skoda = array_find_key($lines, function ($l) {
            $t = trim($l);
            return Str::contains($t, 'Skoda') || Str::contains($t, 'Š k o d a') || Str::contains($t, 'Škoda');
        }) !== null;

        return $has_datum && $has_loading_list && $has_customer_no && $has_skoda;
    }
